...
export function TrialDialogProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <TrialDialogContext.Provider value={{ open, openDemo, close }}>
      {children}
...
function Modal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[520px] max-h-[calc(100dvh-2rem)] overflow-y-auto scrollbar-hide rounded-[24px] bg-white shadow-[0_20px_60px_rgba(0,0,0,0.18)]">
        ...
        <>
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="px-7 pb-7 pt-4">
              ...
              <div className="mt-3">
                <Textarea
                  label="¿Algo que tengamos en cuenta para la demo? (opcional)"
                  name="message"
                  rows={3}
                />
              </div>
              ...
              >
                {pending ? 'Enviando...' : 'Agendar la demo'}
              </button>
